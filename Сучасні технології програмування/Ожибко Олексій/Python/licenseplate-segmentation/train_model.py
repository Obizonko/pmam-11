...
def criterion(inputs, target):
    # Відображення розмірів для відладження
    print(f"sigmoid shape: {inputs['out'].shape}")
    print(f"sigmoid_aux shape: {inputs['aux'].shape}")
    print(f"target shape: {target.shape}")

    # Переконатися, що target має один канал
